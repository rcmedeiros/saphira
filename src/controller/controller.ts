...
export class Controller {
    ...
    protected route(path: string, handler: Handler): void {
        if (handler.params && handler.payload) {
            ...
            if (handler.payload) {
                if ([Type.Object, Type.ObjectArray, Type.StringArray, Type.NumberArray].indexOf(handler.payload.type) === -1) {
            ...
            if ((handler.params !== undefined ? handler.params.length : 0) === handler.action.length) {
                const paths: Array<string> = [`/${this.apiPath}/${this.constructor.name.charAt(0).toLowerCase()}` +
                    `${this.constructor.name.substring(1)}${path !== '' ? '/' + path.charAt(0).toLowerCase() + path.substring(1) : ''}`];
        ...
        new Promise<unknown>((resolve: Function, reject: (reason: unknown) => void): void => {
            ...
            if (!route.params) {
                ...
                if (route.params.every((param: Param) => {
                    const v: unknown = param.name == PAYLOAD ? request.body
                        : param.path || param.parentPath ? (request.params as UnknownObj)[param.name]
                            : route.method === Method.GET || route.method === Method.HEAD || route.method === Method.DELETE ? (request.query as UnknownObj)[param.name]
