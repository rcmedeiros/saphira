...
import {
    SERVICE_1,
    SERVICE_1_NO_PARAMETER,
    SERVICE_1_PATH_PARAMETER,
    SERVICE_1_THROW_ERROR,
    SERVICE_3,
    SERVICE_3_PAGED_LIST,
    SERVICE_4_AN_ARRAY,
    SERVICE_4_AN_OBJECT,
    URI,
} from './setup';
...
describe('Operations', () => {
    ...
    it('should allow path parameters in operations', (done: Done) => {
        chai.request(URI)
            .delete(`${SERVICE_1_PATH_PARAMETER}/4321?sure=yes`)
    ...
    it('should reject required parameters not sent', (done: Done) => {
        chai.request(URI)
            .delete(`${SERVICE_1_PATH_PARAMETER}/4321`)
            .end((err: Error, res: HttpResponse) => {
                if (err) {
                    done(err);
                } else {
    ...
    it('should allow parent path parameters in operations', (done: Done) => {
        chai.request(URI)
            .put(`${SERVICE_1}/9876/parentPathParameter`)
            .send({ what: 'whatever' })
            .end((err: Error, res: HttpResponse) => {
                if (err) {
                    done(err);
    ...
    it('should handle exceptions inside operations', (done: Done) => {
        ...
        promises.push(
            testFailedGET(`${SERVICE_1_THROW_ERROR}1`, 'Something wrong. Throwing.', {
                expectedStatus: HttpStatusCode.INTERNAL_SERVER_ERROR,
                description: '1',
            }),
        );
        promises.push(
            testFailedGET(`${SERVICE_1_THROW_ERROR}2`, 'Something wrong. Rejecting.', {
                expectedStatus: HttpStatusCode.INTERNAL_SERVER_ERROR,
                description: '2',
            }),
        );
        promises.push(
            testFailedGET(`${SERVICE_1_THROW_ERROR}3`, 'Something wrong. Throwing.', {
                expectedStatus: HttpStatusCode.INTERNAL_SERVER_ERROR,
                description: '3',
            }),
        );
        promises.push(
            testFailedGET(`${SERVICE_1_THROW_ERROR}4`, 'Something wrong. Rejecting.', {
                expectedStatus: HttpStatusCode.BAD_GATEWAY,
                description: '4',
            }),
        );
        promises.push(
            testFailedGET(`${SERVICE_1_THROW_ERROR}5`, 'Something wrong. Rejecting.', {
                expectedStatus: HttpStatusCode.INTERNAL_SERVER_ERROR,
                description: '5',
            }),
        );
    ...
    it('operation name should not collide with path parameter', (done: Done) => {
        ...
        promises.push(
            testSuccessfulGET(
                `${SERVICE_3}/subInstances?id=1234`,
                [
                    { id: 2, name: 'Beta' },
                    { id: 3, name: 'Gamma' },
                    { id: 4, name: 'Delta' },
    ...
    it('Should allow objects to answer for more then one verb', (done: Done) => {
        ...
        testSuccessfulPOST(SERVICE_3_PAGED_LIST, {}, ['f', 'g', 'h', 'i', 'j'], 'answer for more than one verb').then(
    ...
    it('Should allow root payloads objects', (done: Done) => {
        const o: SamplePayload = { id: 24, name: 'Omega' };

        testSuccessfulPOST(SERVICE_4_AN_OBJECT, o, o, 'root payload object').then((_responses: Array<HttpResponse>) => {
            done();
        }, done);
    });
    ...
    it('Should allow root payloads objects', (done: Done) => {
        ...
        testSuccessfulPOST(SERVICE_4_AN_ARRAY, a, a, 'root payload array').then((_responses: Array<HttpResponse>) => {
            done();
        }, done);
    });
});
