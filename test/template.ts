        ...
        if (payload[k] !== undefined) {
            switch (typeof payload[k]) {
                ...
                case 'number':
                    ...
                    break;
                case 'object':
                    switch ((payload[k] as unknown).constructor) {
                        case Date:
                        ...
                        case Array: {
                            ...
                            if (aux.length && typeof aux[0] === 'object' && (aux[0] as unknown).constructor !== Date) {
...
export const testSuccessfulGET: TestSuccessfulGET = async (
    endpoint: string,
    expected: unknown,
    description?: string,
): Promise<HttpResponse> =>
    new Promise<HttpResponse>((resolve: Resolution<HttpResponse>, reject: Rejection): void => {
        chai.request(URI)
            .get(endpoint)
            .end((err: Error, res: HttpResponse) => {
                if (err) {
                    ...
                    try {
                        ...
                        expect(res.header['content-type'], description).to.be.equal(
                            `${res.status === HttpStatusCode.OK ? 'application/json' : 'text/plain'}; charset=utf-8`,
                        );
...
declare type TestFailedGET = (
    endpoint: string,
    errorMessage: string,
    opts?: TestFailedGETOpts | string,
) => Promise<HttpResponse>;
export const testFailedGET: TestFailedGET = async (
    endpoint: string,
    errorMessage: string,
    opts?: TestFailedGETOpts | string,
): Promise<HttpResponse> =>
    new Promise<HttpResponse>((resolve: Resolution<HttpResponse>, reject: Rejection): void => {
        const options: TestFailedGETOpts =
            typeof opts === 'string' ? { description: opts, expectedStatus: HttpStatusCode.BAD_REQUEST } : opts;
            ...
            .end((err: Error, res: HttpResponse) => {
                if (err) {
                    ...
                    try {
                        ...
                        expect(res.header['content-type'], options.description).to.be.equal(
...
declare type TestSuccessfulPOST = (
    endpoint: string,
    payload: SamplePayload | Array<SamplePayload>,
    expected: unknown,
    description: string,
) => Promise<Array<HttpResponse>>;
export const testSuccessfulPOST: TestSuccessfulPOST = async (
    endpoint: string,
    payload: SamplePayload | Array<SamplePayload>,
    expected: unknown,
    description: string,
): Promise<Array<HttpResponse>> =>
    new Promise<Array<HttpResponse>>((resolve: Resolution<Array<HttpResponse>>, reject: Rejection): void => {
        const promises: Array<Promise<HttpResponse>> = [];

        promises.push(
            new Promise<HttpResponse>((res: Resolution<HttpResponse>, rej: Rejection): void => {
                    ...
                    .end((err: Error, response: HttpResponse) => {
                        if (err) {
                            ...
                            try {
                                ...
                                expect(JSON.stringify(response.body), description).to.be.equal(
                                ...
                                expect(response.header['content-type'], description).to.be.equal(
                                    'application/json; charset=utf-8',
                                );
                        ...
                        .end((err: Error, response: HttpResponse) => {
                            if (err) {
                                ...
                                try {
                                    ...
                                    expect(response.header['x-high-resolution-elapsed-time'], description).to.not.be
                                        .null;
                                    res(response);
                                } catch (e) {
                                    rej(e);
        ...
        Promise.all(promises).then((responses: Array<HttpResponse>) => {
            resolve(responses);
        }, reject);
    });
...
declare type TestFailedPOST = (
    endpoint: string,
    payload: UnknownPayload,
    errorMessage: string,
    opts?: TailedPOSTOpts | string,
) => Promise<Array<HttpResponse>>;
export const testFailedPOST: TestFailedPOST = async (
    endpoint: string,
    payload: UnknownPayload,
    errorMessage: string,
    opts?: TailedPOSTOpts | string,
                    ...
                    .end((err: Error, response: HttpResponse) => {
                        if (err) {
                            ...
                            try {
                                ...
                                expect(response.header['content-type'], options.description).to.be.equal(
                                    'application/json; charset=utf-8',
                                );
                                expect(response.header['x-high-resolution-elapsed-time'], options.description).to.be
                                    .undefined;
                                res(response);
                            } catch (e) {
                                rej(e);
                    ...
                    chai.request(URI)
                        ...
                        .end((err: Error, response: HttpResponse) => {
                            if (err) {
                                ...
                                try {
                                    options.description = `${options.description} - ${type}`;
                                    expect((response.body as Error).message, options.description).to.be.equal(
                                        errorMessage,
                                    );
                                    expect(response.status, options.description).to.be.equal(
